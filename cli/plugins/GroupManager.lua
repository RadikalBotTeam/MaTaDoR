--Begin GroupManager.lua By @SudoRadical
...
local function delmsg (MaTaDoR,SudoRadical)
    msgs = MaTaDoR.msgs 
    for k,v in pairs(SudoRadical.messages_) do
        msgs = msgs - 1
        tdcli.deleteMessages(v.chat_id_,{[0] = v.id_}, dl_cb, cmd)
        if msgs == 1 then
            tdcli.deleteMessages(SudoRadical.messages_[0].chat_id_,{[0] = SudoRadical.messages_[0].id_}, dl_cb, cmd)
            return false
        end
    end
    tdcli.getChatHistory(SudoRadical.messages_[0].chat_id_, SudoRadical.messages_[0].id_,0 , 100, delmsg, {msgs=msgs})
end
...
💥سکوت کردن فرد (هر پیامی که فرد بدهد پاک میشود(
*Unsilent* `[username , id , reply]`
⚡️بازکردن سکوت بودن فرد 
...
💥سکوت کردن فرد (هر پیامی که فرد بدهد پاک میشود)
*لغو سکوت* `[username , id , reply]`
⚡️بازکردن سکوت بودن فرد 
*اخراج* `[username , id , reply]`
💥اخراج فرد از گروه
*مسدود* `[username , id , reply]`
⚡️مسدود کردن فرد و اجازه ورود مجدد ندادن
*لغو مسدود* `[username , id , reply]`
💥خارج شدن فرد از حالت مسدود 
...
text = [[
`💵 نــرخ فــروش گــروه بــا ربــات
🔖 ʀα∂ι¢αℓвσт  💤
...
3⃣.ربــات داعــمــی نــداریـم بــه عــلــت ڪـیــفــیــت و ڪــارای بــالــای ربــات.
________
بــرای خــریــد بــه ایــدی زیــر مــراجــعــه ڪــنــیــد :`
🆔: @sudoradical
...
--------------------- Welcome -----------------------
	if matches[1] == "welcome" and is_mod(msg) or matches[1] == "Welcome" and is_mod(msg) or matches[1] == "ولکام" and is_mod(msg) then
      ...
      elseif lang then
				return "خوش امد گویی غیر فعال شد_"
          end
			end
		end
	end
